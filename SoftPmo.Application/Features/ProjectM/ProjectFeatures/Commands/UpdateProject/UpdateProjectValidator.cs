...
public sealed class UpdateProjectValidator : AbstractValidator<UpdateProjectCommand>
{
    public UpdateProjectValidator()
    {
        ...
        RuleFor(x => x.PlannedEndDate)
            .GreaterThan(x => x.StartDate).WithMessage("Planlanan bitiş tarihi başlangıç tarihinden sonra olmalıdır.")
            .When(x => x.PlannedEndDate.HasValue);

        RuleFor(x => x.ActualEndDate)
            .GreaterThanOrEqualTo(x => x.StartDate).WithMessage("Gerçek bitiş tarihi başlangıç tarihinden sonra olmalıdır.")
